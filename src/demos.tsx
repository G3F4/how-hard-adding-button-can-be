...
const Toggle = ({
    ...
    <label className="flex items-center gap-2 cursor-pointer select-none">
        ...
        <span className="text-[#707070] text-xs">{label}</span>
    </label>
);
...
export function ButtonVsDivDemo() {
    ...
    const btnStyle =
        "px-4 py-2 rounded-lg text-sm font-semibold bg-[#fd5f2e] text-white cursor-pointer outline-none focus:ring-2 focus:ring-[#fd5f2e] focus:ring-offset-2 focus:ring-offset-[#fefefe]";
    const divStyle =
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center gap-6 flex-wrap">
                    <div className="flex flex-col items-center gap-1">
                        <p className="text-[#06753f] text-xs font-mono mb-1">{"<button>"}</p>
                        <button
                            className={btnStyle}
                    ...
                    <div className="flex flex-col items-center gap-1">
                        <p className="text-[#bb0020] text-xs font-mono mb-1">{"<div onclick>"}</p>
                        <div
                            className={divStyle}
                        ...
                        <p className="text-[#cecece] text-[10px] mt-0.5">not focusable</p>
                    </div>
                </div>

                <div
                    ref={logRef}
                    className="bg-[#f4f4f4] rounded-lg p-3 h-28 overflow-y-auto font-mono text-xs space-y-0.5"
                >
                    {log.length === 0 ? (
                ...
                <button
                    onClick={() => setLog([])}
                    className="text-[#cecece] hover:text-[#8e8e8e] text-xs transition-colors"
                >
                    Clear log
                </button>
            </div>
            ...
            <div className="space-y-3">
                ...
                <div className="relative rounded-lg border border-[#e1e1e1] bg-[#f4f4f4]" style={{ height: 200 }}>
                    ...
                    >
                        <div className="bg-[#e1e1e1] px-3 py-2 flex items-center justify-between border-b border-[#cecece]">
                            <div className="flex items-center gap-2">
                                <span className="text-[#707070] text-xs font-semibold">Header</span>
                            </div>
                            <div className="relative">
                                ...
                                <div
                                    ...
                                    style={{ zIndex: 1000 }}
                                >
                                    <div className="absolute -top-3.5 right-1 text-[8px] font-mono text-[#fd5f2e] bg-[#fefefe] px-1 rounded border border-[#e1e1e1]">
                                        z: 1000
                                    </div>
                                    {["Profile", "Settings", "Logout"].map((item) => (
                                        <div key={item} className="px-3 py-1.5 text-xs text-[#707070]">
                                            {item}
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className={`absolute bottom-0 left-2 text-[8px] font-mono px-1 rounded translate-y-full ${fixTransform ? "text-[#06753f]" : "text-[#bb0020]"}`}>
                    ...
                    <div className="flex items-center justify-center" style={{ height: 160 }}>
                        <span className="text-[#cecece] text-xs">Page content</span>
                    </div>
                    ...
                    >
                        <div className="border border-[#bb0020] text-[#bb0020] rounded px-2 py-1 text-[9px] font-mono bg-[#fefefe]">
                            overlay z: 500
                        </div>
                    </div>
                </div>
                ...
                <p className="text-[#cecece] text-[10px]">
            ...
            <div className="space-y-3">
                ...
                <div className="space-y-1">
                    {rows.map((row) => (
                        ...
                        >
                            <span className="text-[#1d1d1d] text-xs">{row.label}</span>
                            <button
                                onClick={(e) => handleDelete(e, row.id)}
                                className="text-[#bb0020] hover:text-white text-xs px-2 py-0.5 rounded border border-[#bb0020] hover:bg-[#bb0020] transition-colors"
                            >
                                Delete
                            </button>
                        </div>
                    ...
                    {rows.length === 0 && (
                        <p className="text-[#cecece] text-xs text-center py-2">All rows deleted — Reset to restore</p>
                    )}
                </div>

                <div
                    ref={logRef}
                    className="bg-[#f4f4f4] rounded-lg p-2 h-20 overflow-y-auto font-mono text-xs space-y-0.5"
                >
                    {log.length === 0 ? (
    ...
    return (
        <div className="absolute inset-0 bg-black/30 flex items-center justify-center z-10 rounded-lg">
            <div className="bg-[#fefefe] border border-[#bb0020] rounded-xl p-4 w-48 shadow-xl">
                <p className="text-[#bb0020] text-xs font-bold mb-3">Modal (broken)</p>
                <input
                    placeholder="Your name"
                    className="w-full bg-[#f4f4f4] text-[#1d1d1d] text-xs rounded px-2 py-1.5 mb-2 outline-none border border-[#e1e1e1] focus:ring-1 focus:ring-[#8e8e8e]"
                />
                <div className="flex gap-2">
                    ...
                    <button
                        onClick={onClose}
                        className="flex-1 bg-[#f4f4f4] text-[#707070] text-xs py-1 rounded border border-[#e1e1e1] hover:bg-[#e1e1e1]"
                    >
                        Close
                    </button>
                </div>
                <p className="text-[#cecece] text-[10px] mt-2">Tab escapes this modal. No Escape key. Focus won't return.</p>
            </div>
        </div>
    );
}
...
function FixedModal({ onClose, triggerRef }: { onClose: () => void; triggerRef: React.RefObject<HTMLButtonElement | null> }) {
    ...
    return (
        <div className="absolute inset-0 bg-black/30 flex items-center justify-center z-10 rounded-lg">
            ...
            >
                <p className="text-[#06753f] text-xs font-bold mb-3">Modal (fixed)</p>
                <input
                    ref={firstFocusRef}
                    placeholder="Your name"
                    className="w-full bg-[#f4f4f4] text-[#1d1d1d] text-xs rounded px-2 py-1.5 mb-2 outline-none border border-[#e1e1e1] focus:ring-1 focus:ring-[#06753f]"
                />
                <div className="flex gap-2">
                    <button className="flex-1 bg-[#fd5f2e] text-white text-xs py-1 rounded hover:bg-[#e04a1e] focus:outline focus:outline-2 focus:outline-[#06753f]">
                        Submit
                    </button>
                    <button
                        onClick={onClose}
                        className="flex-1 bg-[#f4f4f4] text-[#707070] text-xs py-1 rounded border border-[#e1e1e1] hover:bg-[#e1e1e1] focus:outline focus:outline-2 focus:outline-[#06753f]"
                    >
                        Close
                    </button>
                </div>
                <p className="text-[#06753f] text-[10px] mt-2">Tab cycles inside. Escape closes. Focus returns to button.</p>
            </div>
        </div>
    );
}
        ...
        <DemoFrame label="focus-trap.html">
            <div className="grid grid-cols-2 gap-3 h-full">
                ...
                <div className="flex flex-col gap-2">
                    <p className="text-[#bb0020] text-xs font-bold">❌ Broken</p>
                    <div className="relative bg-[#fefefe] rounded-lg border border-[#e1e1e1] flex-1 flex flex-col items-center justify-center gap-2 p-3" style={{ minHeight: 120 }}>
                        ...
                        <input
                            ...
                            className="w-full bg-[#f4f4f4] text-[#1d1d1d] text-xs rounded px-2 py-1 outline-none border border-[#e1e1e1] focus:ring-1 focus:ring-[#bb0020]"
                        />
                        <button
                ...
                {/* Fixed */}
                <div className="flex flex-col gap-2">
                    <p className="text-[#06753f] text-xs font-bold">✅ Fixed</p>
                    <div className="relative bg-[#fefefe] rounded-lg border border-[#e1e1e1] flex-1 flex flex-col items-center justify-center gap-2 p-3" style={{ minHeight: 120 }}>
                        <p className="text-[#8e8e8e] text-[10px] text-center">Background content (Tab stays in modal)</p>
                        <input
                            placeholder="bg input"
                            className="w-full bg-[#f4f4f4] text-[#1d1d1d] text-xs rounded px-2 py-1 outline-none border border-[#e1e1e1] focus:ring-1 focus:ring-[#06753f]"
    ...
    const ContentLines = () => (
        <div className="space-y-1.5 mb-2">
            {["Dashboard overview", "Total shipments: 142", "Pending: 23"].map((t) => (
                <div key={t} className="bg-[#e1e1e1] rounded px-2 py-1 text-[10px] text-[#8e8e8e]">{t}</div>
            ))}
        </div>
    );
            ...
            <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                        <p className="text-[#bb0020] text-[10px] font-bold">No placeholder</p>
                        <div className="bg-[#f4f4f4] rounded-lg p-2 border border-[#e1e1e1]" style={{ minHeight: 110 }}>
                            ...
                            {phase === "waiting" && (
                                <div className="flex items-center gap-1">
                                    ...
                                    <span className="text-[#cecece] text-[9px] ml-1">loading...</span>
                                </div>
                            )}
                        </div>
                        {phase === "shifted" && (
                            <p className="text-[#bb0020] text-[9px] font-mono">↑ content jumped! CLS +0.15</p>
                        )}
                    </div>

                    {/* Fixed */}
                    <div className="flex flex-col gap-1">
                        <p className="text-[#06753f] text-[10px] font-bold">With placeholder</p>
                        <div className="bg-[#f4f4f4] rounded-lg p-2 border border-[#e1e1e1]" style={{ minHeight: 110 }}>
                            <ContentLines />
                            <div className="relative">
                        ...
                        {fixedPhase === "appeared" && (
                            <p className="text-[#06753f] text-[9px] font-mono">no shift — CLS: 0</p>
                        )}
                    </div>
                </div>
                ...
                <button
                    onClick={replay}
                    className="text-[#cecece] hover:text-[#8e8e8e] text-xs transition-colors border border-[#e1e1e1] px-3 py-1 rounded"
                >
                    Replay animation
                </button>
            </div>
