...
const LayerBar = ({ active }: { active: number }) => {
    ...
    return (
        <div className="absolute top-4 left-6 flex items-center gap-1.5">
            {layers.map((l, i) => (
                <div key={i} className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs transition-all ${i === active ? `${l.bg} ${l.border} border ${l.color} font-bold` : i < active ? "bg-[#e1e1e1] border border-[#cecece] text-[#707070]" : "text-[#cecece]"}`}>
                    <span>{l.icon}</span>
...
const ModuleBar = ({ moduleId }: { moduleId: number }) => (
    <div className="absolute top-4 left-6 flex items-center gap-1.5">
        {modules.map((m) => (
            <div key={m.id} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold transition-all ${m.id === moduleId ? `${m.color} ${m.text}` : m.id < moduleId ? "bg-[#e1e1e1] text-[#707070]" : "bg-[#f4f4f4] text-[#cecece]"}`}>
                <span>{m.id}</span>
...
const slides = [
    // ============ TITLE ============
    {
        ...
        render: () => (
            <div className="flex flex-col items-center justify-center h-full text-center px-8">
                ...
                <h1 className="text-4xl font-bold text-[#1d1d1d] mb-6 leading-tight">"How hard can it be<br/>to add a button on a website?"</h1>
                <p className="text-lg text-[#707070] max-w-2xl leading-relaxed">Because sometimes one button is the start of an architectural discussion about <span className="text-[#fd5f2e]">scalability</span>, <span className="text-[#1767e8]">responsibility</span>, and the <span className="text-[#946810]">borders of frontend</span>.</p>
                <div className="mt-16 text-[#cecece] text-sm">→ use arrow keys or click navigation below</div>
            </div>
        ),
    },
    // ============ MODULE 1 — BUTTON PROBLEMS ============
    {
        ...
        render: () => (
            <div className="flex flex-col items-center justify-center h-full text-center px-8">
                ...
                <h2 className="text-4xl font-bold text-[#1d1d1d] mb-4">"It's just a button"</h2>
                <p className="text-xl text-[#707070]">Five problems hiding behind a simple button.<br/>Each one is a different web specialization.</p>
            </div>
        ),
    },
        ...
        render: () => (
            <div className="flex gap-6 h-full px-10 py-6">
                ...
                <div className="w-[42%] flex flex-col justify-center">
                    ...
                    <h2 className="text-2xl font-bold text-[#1d1d1d] mb-2">{"<button>"} vs {"<div>"}</h2>
                    <p className="text-[#707070] text-sm mb-5">"They look the same. They behave completely differently."</p>
                    <div className="space-y-3">
                        <div>
                            <p className="text-[#06753f] font-bold text-sm mb-1.5">{"<button>"} gives you for free:</p>
                        ...
                        <div>
                            <p className="text-[#bb0020] font-bold text-sm mb-1.5">{"<div onclick>"} needs manual work:</p>
                            <div className="space-y-1 text-xs text-[#707070]">
                        ...
                        <p className="text-[#8e8e8e] text-xs border-t border-[#e1e1e1] pt-3">~15 lines to recreate what <span className="font-mono">{"<button>"}</span> does automatically. Most projects skip half. This is <span className="text-[#fd5f2e]">HTML standards</span> knowledge.</p>
                    </div>
                </div>
                {/* Right: demo */}
        ...
        render: () => (
            <div className="flex gap-6 h-full px-10 py-6">
                ...
                <div className="w-[42%] flex flex-col justify-center">
                    ...
                    <h2 className="text-2xl font-bold text-[#1d1d1d] mb-2">Stacking context</h2>
                    <p className="text-[#707070] text-sm mb-5">"Your button is there. Something invisible is covering it."</p>
                    <div className="space-y-3">
                        <div className="bg-[#fefefe] border border-[#e1e1e1] rounded-xl p-4 shadow-sm">
                            <p className="text-[#1d1d1d] font-bold text-xs mb-2">The scenario</p>
                        ...
                        <div className="bg-[#fefefe] border border-[#e1e1e1] rounded-xl p-4 shadow-sm">
                            <p className="text-[#bb0020] font-bold text-xs mb-2">The catch</p>
                            <div className="space-y-1.5 text-xs text-[#707070]">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-1">
                                {["transform", "opacity < 1", "filter", "will-change", "position: fixed"].map(p => (
                                    <span key={p} className="font-mono text-[10px] bg-[#e1e1e1] px-1.5 py-0.5 rounded text-[#707070]">{p}</span>
                                ))}
                            </div>
                        </div>
                        <p className="text-[#8e8e8e] text-xs border-t border-[#e1e1e1] pt-3">z-index is not a global ranking — it's local to a stacking context.</p>
                    </div>
                </div>
                {/* Right: demo */}
                <div className="flex-1 flex flex-col">
        ...
        render: () => (
            <div className="flex gap-6 h-full px-10 py-6">
                ...
                <div className="w-[42%] flex flex-col justify-center">
                    ...
                    <h2 className="text-2xl font-bold text-[#1d1d1d] mb-2">Event propagation</h2>
                    <p className="text-[#707070] text-sm mb-5">"You click 'Delete'. The row also opens. Why?"</p>
                    <div className="space-y-3">
                        <div className="bg-[#fefefe] border border-[#e1e1e1] rounded-xl p-4 shadow-sm">
                            <p className="text-[#bc1cf2] font-bold text-xs mb-2">The bubbling trap</p>
                            <div className="space-y-1.5 text-xs text-[#707070]">
                        ...
                        <p className="text-[#8e8e8e] text-xs border-t border-[#e1e1e1] pt-3">DOM events have 3 phases: capture → target → bubble. Most devs only know bubble. Full model = deep <span className="text-[#fd5f2e]">JS/DOM knowledge</span>.</p>
                    </div>
                </div>
                {/* Right: demo */}
                <div className="flex-1 flex flex-col">
        ...
        render: () => (
            <div className="flex gap-6 h-full px-10 py-6">
                ...
                <div className="w-[42%] flex flex-col justify-center">
                    ...
                    <h2 className="text-2xl font-bold text-[#1d1d1d] mb-2">Focus trap & keyboard nav</h2>
                    <p className="text-[#707070] text-sm mb-5">"Try using your site with just the keyboard. It probably doesn't work."</p>
                    <div className="space-y-3">
                        <div className="bg-[#fefefe] border border-[#e1e1e1] rounded-xl p-4 shadow-sm">
                            <p className="text-[#bb0020] font-bold text-xs mb-2">❌ What usually happens</p>
                            <div className="space-y-1 text-xs text-[#707070]">
                        ...
                        <div className="bg-[#fefefe] border border-[#e1e1e1] rounded-xl p-4 shadow-sm">
                            <p className="text-[#06753f] font-bold text-xs mb-2">✅ Fix (pure HTML + JS)</p>
                            <div className="space-y-1 text-xs text-[#707070]">
                        ...
                        <p className="text-[#8e8e8e] text-xs border-t border-[#e1e1e1] pt-3">15% of users rely on keyboard navigation. WCAG 2.1 AA is a legal requirement in the EU. Accessibility is its own <span className="text-[#fd5f2e]">specialization</span>.</p>
                    </div>
                </div>
                {/* Right: demo */}
                <div className="flex-1 flex flex-col">
        ...
        render: () => (
            <div className="flex gap-6 h-full px-10 py-6">
                ...
                <div className="w-[42%] flex flex-col justify-center">
                    ...
                    <h2 className="text-2xl font-bold text-[#1d1d1d] mb-2">Layout Shift</h2>
                    <p className="text-[#707070] text-sm mb-5">"You add a button and suddenly everything 'jumps'."</p>
                    <div className="space-y-3">
                        <div>
                            <p className="text-[#1d1d1d] font-semibold text-xs mb-1.5">Button text changes size:</p>
                            <div className="flex items-center gap-2 flex-wrap">
                                <div className="bg-[#1767e8] text-white px-3 py-1 rounded text-xs">"Download"</div>
                                <span className="text-[#cecece] text-xs">→</span>
                        ...
                        <div className="bg-[#fefefe] border border-[#e1e1e1] rounded-xl p-4 shadow-sm">
                            <p className="text-[#06753f] font-bold text-xs mb-2">Solutions:</p>
                            <div className="space-y-1 text-xs text-[#707070]">
                        ...
                        <p className="text-[#8e8e8e] text-xs border-t border-[#e1e1e1] pt-3">CLS is a Core Web Vital. Google uses it for search ranking. Web performance = its own <span className="text-[#fd5f2e]">specialization</span>.</p>
                    </div>
                </div>
                {/* Right: demo */}
                <div className="flex-1 flex flex-col">
    ...
    // M1 - Bridge
    {
        ...
        render: () => (
            <div className="flex flex-col items-center justify-center h-full text-center px-12">
                <h2 className="text-3xl font-bold text-[#1d1d1d] mb-6">And that was "just" the UI layer.</h2>
                <p className="text-xl text-[#707070] mb-10 max-w-2xl leading-relaxed">HTML semantics, CSS rendering, DOM events,<br/>accessibility, web performance...</p>
                <p className="text-lg text-[#707070] max-w-2xl leading-relaxed mb-6">Each of these five problems is someone's <span className="text-[#fd5f2e] font-semibold">specialization</span>.<br/>And we haven't even left the browser yet.</p>
                <p className="text-xl text-[#707070] max-w-2xl leading-relaxed">Now what happens when that button triggers<br/><span className="text-[#1767e8] font-bold text-2xl">a real business process</span><br/>that cuts through the entire stack?</p>
    ...
    // ============ MODULE 2 — FULL STACK PROCESS ============
    {
        ...
        render: () => (
            <div className="flex flex-col items-center justify-center h-full text-center px-8">
                ...
                <h2 className="text-4xl font-bold text-[#1d1d1d] mb-4">"Download Report"</h2>
                <p className="text-xl text-[#707070]">A new process in an existing system.<br/>Layer by layer — from click to database.</p>
            </div>
        ),
    },
    // M2 - PM scene
            ...
            <div className="flex flex-col justify-center h-full px-16">
                ...
                <div className="bg-[#fefefe] border border-[#e1e1e1] rounded-2xl p-8 max-w-3xl shadow-sm">
                    <div className="flex items-start gap-4 mb-6">
                        <div className="w-10 h-10 rounded-full bg-[#fd5f2e] flex items-center justify-center text-white font-bold text-sm flex-shrink-0">PM</div>
                        <p className="text-[#1d1d1d] text-lg leading-relaxed">"Users want to <span className="text-[#946810] font-bold">download a PDF report</span> from the dashboard. We have the data, we have the view — let's add a button. How long will it take?"</p>
                    </div>
                    <div className="flex items-start gap-4">
                        ...
                        <p className="text-[#707070] text-lg">"It depends..."</p>
                    </div>
                </div>
                <div className="mt-8 text-[#8e8e8e] text-sm max-w-3xl">A PDF report is a new process — new data flow, new transformations, new output. It has to pass through <span className="text-[#1d1d1d]">5 architectural layers</span>. At each one — decisions.</div>
            </div>
        ),
    },
    // M2 - Map
    {
        ...
        render: () => (
            <div className="flex flex-col justify-center h-full px-16">
                <h2 className="text-3xl font-bold text-[#1d1d1d] mb-4">Journey map of a new process</h2>
                <p className="text-[#707070] mb-8">5 layers. At each — different questions, different decisions, different people.</p>
    ...
    // M2 - Layer 1: UI/UX
    {
        ...
        render: () => (
            <div className="flex flex-col justify-center h-full px-16">
                ...
                <h2 className="text-4xl font-bold text-[#1d1d1d] mb-3">🎨 UI / UX</h2>
                <p className="text-[#707070] text-lg mb-8">Before we write a single line of code — the designer asks:</p>
                <div className="bg-[#fefefe] border border-[#e1e1e1] rounded-2xl p-8 max-w-3xl shadow-sm">
                    <div className="space-y-4">
                        {[
                            ...
                            <div key={i} className="border-l-2 border-[#1767e8] pl-4">
                                <p className="text-[#1d1d1d] font-semibold">{item.q}</p>
    ...
    // M2 - Layer 2: Client App
    {
        ...
        render: () => (
            <div className="flex flex-col justify-center h-full px-16">
                <p className="text-[#bc1cf2] text-sm uppercase tracking-widest mb-2">Layer 2</p>
                <h2 className="text-4xl font-bold text-[#1d1d1d] mb-3">⚛️ Client Application</h2>
                <p className="text-[#707070] text-lg mb-8">User clicked. Now the frontend has a series of decisions:</p>
                <div className="bg-[#fefefe] border border-[#e1e1e1] rounded-2xl p-8 max-w-3xl shadow-sm">
                    <div className="space-y-4">
                        {[
    ...
    // M2 - Layer 3: BFF
    {
        ...
        render: () => (
            <div className="flex flex-col justify-center h-full px-16">
                ...
                <h2 className="text-4xl font-bold text-[#1d1d1d] mb-3">🔀 Backend for Frontend</h2>
                <p className="text-[#707070] text-lg mb-8">The translator between the UI world and the domain world.</p>
                <div className="bg-[#fefefe] border border-[#e1e1e1] rounded-2xl p-8 max-w-3xl shadow-sm">
                    <div className="space-y-4">
                        {[
    ...
    // M2 - Layer 4: Domain Service
    {
        ...
        render: () => (
            <div className="flex flex-col justify-center h-full px-16">
                <p className="text-[#e00098] text-sm uppercase tracking-widest mb-2">Layer 4</p>
                <h2 className="text-4xl font-bold text-[#1d1d1d] mb-3">⚙️ Domain Service</h2>
                <p className="text-[#707070] text-lg mb-8">This is where the heavy lifting happens.</p>
                <div className="bg-[#fefefe] border border-[#e1e1e1] rounded-2xl p-8 max-w-3xl shadow-sm">
                    <div className="space-y-4">
                        {[
    ...
    // M2 - Layer 5: DB
    {
        ...
        render: () => (
            <div className="flex flex-col justify-center h-full px-16">
                <p className="text-[#946810] text-sm uppercase tracking-widest mb-2">Layer 5</p>
                <h2 className="text-4xl font-bold text-[#1d1d1d] mb-3">🗄️ Database</h2>
                <p className="text-[#707070] text-lg mb-8">The foundation. This is where performance truth lives.</p>
                <div className="bg-[#fefefe] border border-[#e1e1e1] rounded-2xl p-8 max-w-3xl shadow-sm">
                    <div className="space-y-4">
                        {[
    ...
    // M2 - Error handling
    {
        ...
        render: () => (
            <div className="flex flex-col justify-center h-full px-16">
                <h2 className="text-3xl font-bold text-[#1d1d1d] mb-3">What happens when things go wrong?</h2>
                <p className="text-[#707070] mb-5">Each layer fails in its own way:</p>
                <div className="space-y-2.5 max-w-3xl">
                    {[
                        ...
                        <div key={i} className="bg-[#ffe2e6] border border-[#bb0020]/20 rounded-xl p-3.5 flex items-start gap-3">
                            <span className="text-base flex-shrink-0">{item.layer}</span>
                            <div>
                                <p className="text-[#bb0020] font-semibold text-sm">{item.err}</p>
    ...
    // ============ MODULE 3 — NARROW SPECIALIZATION ============
    {
        ...
        render: () => (
            <div className="flex flex-col items-center justify-center h-full text-center px-8">
                <p className="text-[#946810] text-sm uppercase tracking-widest mb-4">Module 3</p>
                <h2 className="text-4xl font-bold text-[#1d1d1d] mb-4">Narrow Specialization</h2>
                <p className="text-xl text-[#707070]">Why "I'm a frontend developer"<br/>stopped being enough — and what to do about it.</p>
            </div>
        ),
    },
        ...
        render: () => {
            ...
            return (
                <div className="flex flex-col justify-center h-full px-10">
                    <h2 className="text-2xl font-bold text-[#1d1d1d] mb-1">The phenomenon: narrow specialization</h2>
                    <p className="text-[#707070] text-sm mb-5">From one generalist role to dozens of specialists — and why the market is now correcting.</p>
                    ...
                    <div className="grid grid-cols-5 gap-3">
                        {eras.map((e, i) => (
                            <div key={i} className={`${e.bg} border ${e.border} rounded-xl p-3.5`}>
                                ...
                                <p className="text-[#1d1d1d] font-semibold text-xs mb-2.5">{e.label}</p>
    ...
    // M3 - The market shift (supply vs demand chart)
    {
        ...
        render: () => {
            ...
            return (
                <div className="flex flex-col justify-center h-full px-10">
                    <h2 className="text-2xl font-bold text-[#1d1d1d] mb-1">The market shift</h2>
                        ...
                        <svg viewBox="0 0 800 230" className="w-full">
                            ...
                            {[20, 40, 60, 80, 100].map(v => (
                                <g key={v}>
                                    ...
                                    <text x={cL - 6} y={py(v) + 3} textAnchor="end" fill="#8e8e8e" fontSize={9} fontFamily="monospace">{v}</text>
                                </g>
                            ))}
                            {/* Area fills */}
                            ...
                            <text x={px(9) - 10} y={py(100) - 8} textAnchor="end" fill="#06753f" fontSize={9} fontWeight="bold">Peak demand</text>
                            ...
                            <text x={px(9.92) + 8} y={cT + 10} textAnchor="start" fill="#bb0020" fontSize={8} opacity={0.8}>supply overtakes demand</text>
                            {/* Gap labels */}
                            <text x={(px(5) + px(8)) / 2} y={py(80)} textAnchor="middle" fill="#06753f" fontSize={9} opacity={0.5}>Demand &gt; Supply</text>
                            <text x={(px(11) + px(13)) / 2} y={py(62)} textAnchor="middle" fill="#bb0020" fontSize={9} opacity={0.5}>Supply &gt; Demand</text>
                            {/* X-axis year labels */}
                            {years.map((yr, i) => (
                                <text key={i} x={px(i)} y={cB + 16} textAnchor="middle" fill="#8e8e8e" fontSize={9}>{yr}</text>
                            ))}
                        </svg>
                        {/* Legend */}
                    ...
                    <div className="flex gap-8 mt-4 max-w-5xl">
                        <div className="text-center">
                            <p className="text-[#06753f] text-xl font-bold">2x</p>
                            <p className="text-[#8e8e8e] text-[10px]">job postings growth<br/>2019 &rarr; 2021</p>
                        </div>
                        <div className="text-center">
                            <p className="text-[#bb0020] text-xl font-bold">&minus;60%</p>
                            <p className="text-[#8e8e8e] text-[10px]">job postings drop<br/>2021 &rarr; 2023</p>
                        </div>
                        <div className="text-center">
                            <p className="text-[#946810] text-xl font-bold">2022</p>
                            <p className="text-[#8e8e8e] text-[10px]">year supply overtook<br/>demand</p>
                        </div>
                        <div className="text-center">
                            <p className="text-[#1d1d1d] text-xl font-bold">260k+</p>
                            <p className="text-[#8e8e8e] text-[10px]">tech layoffs<br/>in 2023</p>
                        </div>
                    </div>
                    <p className="text-[#cecece] text-[10px] mt-3 max-w-5xl">Based on aggregated trends from LinkedIn Economic Graph, Indeed Hiring Lab, Stack Overflow Developer Surveys (2012–2025), and layoffs.fyi.</p>
                </div>
            );
        },
    },
    // M3 - How it happened
    {
        ...
        render: () => (
            <div className="flex flex-col justify-center h-full px-16">
                <h2 className="text-3xl font-bold text-[#1d1d1d] mb-3">How did we get here?</h2>
                <p className="text-[#707070] mb-8">The mechanisms that created narrow specialists.</p>
                <div className="space-y-4 max-w-3xl">
                    {[
                        ...
                        <div key={i} className="flex items-start gap-4 bg-[#fefefe] border border-[#e1e1e1] rounded-xl p-4 shadow-sm">
                            ...
                            <div>
                                <p className="text-[#1d1d1d] font-semibold">{item.t}</p>
                                <p className="text-[#8e8e8e] text-sm mt-1">{item.d}</p>
                            </div>
                        </div>
                    ))}
    ...
    // M3 - The problem
    {
        ...
        render: () => (
            <div className="flex flex-col justify-center h-full px-16">
                <h2 className="text-3xl font-bold text-[#1d1d1d] mb-3">Why is this a problem?</h2>
                <p className="text-[#707070] mb-8">The narrowly specialized developer:</p>
                <div className="grid grid-cols-2 gap-5 max-w-4xl">
                    <div className="bg-[#ffe2e6] border border-[#bb0020]/20 rounded-xl p-5">
                        <p className="text-[#bb0020] font-bold text-lg mb-3">Doesn't understand context</p>
                        <p className="text-[#707070] text-sm">Writes a button but has no idea what happens after the click. Doesn't know the API contract, doesn't understand the data model, can't see the DB performance implications.</p>
                    </div>
                    <div className="bg-[#ffe2e6] border border-[#bb0020]/20 rounded-xl p-5">
                        <p className="text-[#bb0020] font-bold text-lg mb-3">Makes bad decisions</p>
                        <p className="text-[#707070] text-sm">Implements PDF generation on the frontend because they don't know the backend has infrastructure for it. Or builds polling because they don't know we have WebSockets.</p>
                    </div>
                    <div className="bg-[#ffe2e6] border border-[#bb0020]/20 rounded-xl p-5">
                        <p className="text-[#bb0020] font-bold text-lg mb-3">Is hard to transfer</p>
                        <p className="text-[#707070] text-sm">Knows one framework, one ecosystem, one toolchain. Switching projects = learning from scratch. A risk for the company and the career.</p>
                    </div>
                    <div className="bg-[#ffe2e6] border border-[#bb0020]/20 rounded-xl p-5">
                        <p className="text-[#bb0020] font-bold text-lg mb-3">Doesn't grow as an engineer</p>
                        <p className="text-[#707070] text-sm">Senior doesn't mean "5 years of React." Senior means understanding trade-offs across multiple levels. Narrow specialization blocks that growth.</p>
                    </div>
                </div>
            </div>
        ),
    ...
    // M3 - What to do
    {
        ...
        render: () => (
            <div className="flex flex-col justify-center h-full px-16">
                <h2 className="text-3xl font-bold text-[#1d1d1d] mb-3">What to do about it?</h2>
                <p className="text-[#707070] mb-8">Practical steps to avoid becoming a one-button specialist.</p>
                <div className="space-y-4 max-w-3xl">
                    {[
                        ...
                        <div key={i} className="flex items-start gap-4">
                            <span className="text-[#946810] font-mono font-bold text-lg">{item.n}</span>
                            <div>
                                <p className="text-[#1d1d1d] font-semibold text-lg">{item.t}</p>
                                <p className="text-[#707070] text-sm">{item.d}</p>
                            </div>
                        </div>
                    ))}
                </div>
    ...
    // M3 - T-shape
    {
        ...
        render: () => (
            <div className="flex flex-col justify-center items-center h-full px-16 text-center">
                <h2 className="text-3xl font-bold text-[#1d1d1d] mb-8">The T-shaped developer</h2>
                <div className="max-w-2xl w-full">
                    <div className="flex justify-center mb-1">
                        <div className="bg-[#fff9cf] border border-[#efb503] rounded-t-xl px-4 py-3 w-full text-center">
                            <p className="text-[#946810] font-bold text-sm">Broad understanding: UX · API · DB · Infra · Security · Product</p>
                        </div>
                    </div>
                ...
                <p className="text-[#707070] mt-8 max-w-lg text-sm leading-relaxed">It's not about being a full-stack developer. It's about <span className="text-[#1d1d1d]">understanding the context</span> around your specialization. Deep knowledge + broad understanding = better decisions at every layer.</p>
            </div>
        ),
    },
    ...
    // ============ CLOSING ============
    {
        ...
        render: () => (
            <div className="flex flex-col justify-center h-full px-16">
                <h2 className="text-3xl font-bold text-[#1d1d1d] mb-8">🎯 Summary</h2>
                <div className="space-y-6 max-w-2xl">
                    <div className="flex items-start gap-4">
                        <div className="w-3 h-3 rounded-full bg-[#fd5f2e] mt-2 flex-shrink-0" />
                        <div>
                            <p className="text-[#1d1d1d] font-semibold text-lg">Even a "simple" button is complex</p>
                            <p className="text-[#707070] text-sm">HTML semantics, CSS rendering, DOM events, accessibility, web performance — five specializations hiding behind one button.</p>
                        </div>
                    </div>
                    <div className="flex items-start gap-4">
                        <div className="w-3 h-3 rounded-full bg-[#1767e8] mt-2 flex-shrink-0" />
                        <div>
                            <p className="text-[#1d1d1d] font-semibold text-lg">A new process = decisions at every layer</p>
                            <p className="text-[#707070] text-sm">From UI through client, BFF, domain service to database — and decisions propagate both up and down.</p>
                        </div>
                    </div>
                    <div className="flex items-start gap-4">
                        <div className="w-3 h-3 rounded-full bg-[#ffcc05] mt-2 flex-shrink-0" />
                        <div>
                            <p className="text-[#1d1d1d] font-semibold text-lg">Understanding context &gt; narrow expertise</p>
                            <p className="text-[#707070] text-sm">T-shaped mindset — deep knowledge in your domain, broad understanding of the rest of the stack.</p>
                        </div>
                    </div>
                </div>
                <div className="mt-10 border-t border-[#e1e1e1] pt-6 max-w-2xl">
    ...
    return (
        <div className="w-full h-screen bg-[#f4f4f4] relative overflow-hidden select-none">
            ...
            <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between px-6 py-3">
                <button onClick={() => go(-1)} disabled={current === 0} className="text-[#8e8e8e] hover:text-[#1d1d1d] disabled:opacity-20 transition text-xl px-3 py-1">←</button>
                ...
                <button onClick={() => go(1)} disabled={current === total - 1} className="text-[#8e8e8e] hover:text-[#1d1d1d] disabled:opacity-20 transition text-xl px-3 py-1">→</button>
            </div>
